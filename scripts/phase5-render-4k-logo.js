...
const main = async () => {
  ...
  console.log('[phase5-hq-logo] Render complete');
  console.log(`[phase5-hq-logo] Output: ${path.relative(PROJECT_ROOT, result.out)}`);
  console.log(
    `[phase5-hq-logo] Video: ${result.width}x${result.height} @ ${result.fps}fps, ${result.durationInSeconds.toFixed(2)}s`,
  );
};

main().catch((error) => {
  console.error(`[phase5-hq-logo] Failed: ${error.message || error}`);
  process.exit(1);
});
