...
const parseArgs = (argv) => {
  const args = {
    ...
    out: path.join(PROJECT_ROOT, 'output', 'phase5-video-16x9.mp4'),
...
const main = async () => {
  ...
  const result = await runPhase5Render({
    ...args,
    compositionId: 'LyricsTemplateVideo16x9',
  });

  console.log('[phase5-16x9] Render complete');
  console.log(`[phase5-16x9] Output: ${path.relative(PROJECT_ROOT, result.out)}`);
  console.log(
    `[phase5-16x9] Video: ${result.width}x${result.height} @ ${result.fps}fps, ${result.durationInSeconds.toFixed(2)}s`,
  );
};

main().catch((error) => {
  console.error(`[phase5-16x9] Failed: ${error.message || error}`);
  process.exit(1);
});
