...
const LOGO_REVEAL_FADE_SECONDS = 0.9;
...
const getLogoAnimationState = ({frame, fps, width, height}) => {
  ...
  return {
    ...
    revealStartFrame: introFrames + moveFrames,
  };
};
...
export const LyricsTemplateVideo = ({songData, showAnimatedLogo = false, logoSrc}) => {
  ...
  const contentRevealOpacity = useMemo(() => {
    ...
    const revealEndFrame =
      logoState.revealStartFrame + Math.max(1, Math.round(LOGO_REVEAL_FADE_SECONDS * fps));

    return interpolate(frame, [logoState.revealStartFrame, revealEndFrame], [0, 1], {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
    });
  }, [frame, fps, logoState.revealStartFrame, showAnimatedLogo]);
    ...
    >
      ...
      >
        <Img
          ...
          style={{
            ...
            height: '100%',
            objectFit: 'cover',
            opacity: 0.6 * contentRevealOpacity,
            mixBlendMode: 'screen',
      ...
      <svg
        ...
        style={{
          ...
          pointerEvents: 'none',
          zIndex: 10,
          opacity: 0.7 * contentRevealOpacity,
        }}
      >
        <defs>
          {waveLayers.map((wave) => {
        ...
        >
          <div
            style={{
              ...
              background: `rgba(11, 14, 22, ${Math.max(logoState.plateOpacity, 0.36)})`,
